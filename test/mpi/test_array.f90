...
if(mpi_id == 0) print *, 'PASSED: array write'

call test_read_slice('test_array.h5')
if(mpi_id == 0) print *, 'PASSED: slice read'

call test_write_slice('test_array.h5')
if(mpi_id == 0) print *, 'PASSED: slice write'

call test_readwrite_array('test_group_array.h5', ng=69, nn=100, pn=5)
if(mpi_id == 0) print *,'PASSED: array write / read'
...
if(mpi_id == 0) print *, 'PASSED: overwrite slice 1d, stride=1'
...
if(mpi_id == 0) print *, 'PASSED: overwrite slice 1d, no stride'

dims2 = [4,4]

call h%create('/int32a-2d', dtype=H5T_NATIVE_INTEGER, dset_dims=dims2)
if(mpi_id == 0) print *, 'create and write slice 2d, stride=1'
...
call h%open(filename, action='rw', mpi=.true.)
