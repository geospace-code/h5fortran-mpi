...
real(real32), intent(inout) :: A2(:,:), A3(:,:,:)
...
if(mpi_id == mpi_root_id) then
  ...
  allocate(t2(lx1,lx2), t3(lx1,lx2,lx3))
...
if(mpi_id == mpi_root_id) then
  do i = 1, Nmpi-1
    call mpi_send(t2(i*dx1+1:(i+1)*dx1, :), dx1*lx2, MPI_REAL, i, tagA2, mpi_h5comm, ierr)
  ...
  call mpi_recv(A2, dx1*lx2, MPI_REAL, mpi_root_id, tagA2, mpi_h5comm, MPI_STATUS_IGNORE, ierr)
